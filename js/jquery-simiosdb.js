(function($, undefined) {
	'use strict';
	var simiosDB = window.indexedDB || window.mozIndexedDB || window.webkitIndexedDB || window.msIndexedDB;
	...
	$.extend({
		...
		"simiosDB": function(dbName, config) {
			...
			var dbPromise = wrap.request(function() {
				...
				return version ? simiosDB.open(dbName, parseInt(version)) : simiosDB.open(dbName);
			});
					...
					return $.Deferred(function(dfd) {
						dbPromise.then(function(db, e) {
							...
							wrap.request(function() {
								return simiosDB.deleteDatabase(dbName);
							...
							me.transaction(storeName, getDefaultTransaction(mode)).then(function() {
								...
								if (err.code === err.NOT_FOUND_ERR && (mode === true || typeof mode === "object")) {
									...
									dbPromise = wrap.request(function() {
										...
										return simiosDB.open(dbName, (parseInt(db.version, 10) || 1) + 1);
									});
									dbPromise.then(function(db, e) {
	...
	$.simiosDB.IDBCursor = IDBCursor;
	$.simiosDB.IDBTransaction = IDBTransaction;
	$.idb = $.simiosDB;
})(jQuery);
